...
(global-hl-line-mode 1)     ; Highlight line where the cursor is
(transient-mark-mode 1)     ; Highlights the "mark" region
...
(setq org-todo-keywords
	...
	(sequence "PLAN(p)" "REVIEW(v)" "WAIT(w@/!)" "HOLD(h)" "|" "COMPLETED(c)" "CANC(k@)")))
...
(setq org-capture-templates
       '(("t" "Table Entry" table-line (file+headline "~/org/cycling/fit-utils.org" "Weight & Energy Tracker")
         "| %U | %^{Lbs} | 35 | %^{Fat%} | 243 | | | |":table-line-pos "II+1" :immediate-finish t)

	 ("l" "Todo" entry (file+headline "~/org/inbox.org" "Story Board")
              "* TODO %U -  %?" :prepend t)

	 ("w" "Weekly Cycling Plan" entry
          (file+headline "~/org/cycling/cycling.org" "Weekly Plans")
          "* Week of %^t\n:PROPERTIES:\n:GOAL: 3.0 W/kg\n:END:\n\n** Monday\n- Lift (heavy)\n- Optional 30–45 min Z2\n\n** Tuesday\n- Threshold session\n\n** Wednesday\n- Z2 aerobic\n\n** Thursday\n- Punch / VO2 session\n\n** Friday\n- Light lift\n\n** Saturday\n- Race or simulation\n\n** Sunday\n- 3 hr aerobic\n\nNotes:\n- Weight:\n- Fatigue:\n- Adjustments:\n")

        ("d" "Daily Workout Log" entry
         (file+headline "~/org/cycling/cycling.org" "Workout Log")
         "* %U - %^{Workout Type}\n:PROPERTIES:\n:TSS: %^{TSS}\n:WEIGHT: %^{Weight}\n:END:\n\n** Plan\n%?\n\n** Actual\n\n** Notes\n- RPE:\n- Recovery:\n- Nutrition:\n")

        ("p" "Phase Planning Block" entry
         (file+headline "~/org/cycling/cycling.org" "Season Plan")
         "* %^{Phase Name}\n:PROPERTIES:\n:START: %^t\n:END: %^t\n:END:\n\n** Objectives\n- \n\n** Focus Workouts\n- \n\n** Weight Target\n- \n\n** Notes\n%?\n")

        ("c" "Weight Check-in" entry
         (file+headline "~/org/cycling/cycling.org" "Weight Tracking")
         "* %U\n- Weight: %^{Weight} lb\n- Weekly Trend:\n- Notes:\n")))
...
(setq org-babel-python-command "python3") ; Replace "python3" with your executable path
...
(use-package org-roam
  ...
  ;;  '(("d" "default" plain
  ;;     "%?" 
  ;;     :if-new (file+head "%<%Y%m%d%H%M%S> - %{slug}.org" "#+titlfe: %{title}\n")
  ;;     :unnarrowed t)))
...
;; (use-package python
;;   :ensure nil
;;   :hook (python-mode . eglot-hover-mode) ; Use built-in python-mode with eglot
;;   :config
;;   (setq python-shell-interpreter "python3") ; Set default python interpreter
;;   (setq explicit-shell-file-name "/bin/bash") ; Use a proper shell for shell commands
;;   )

;; (use-package eglot
;;   :ensure t
;;   :hook (python-mode . eglot-mode) ; Activate eglot for python files
;;   :config
;;   ;; Optional: Enable formatting on save (requires a formatter like ruff-lsp)
...
;;   :ensure t
;;   :config
;;   (pyvenv-mode 1)) ; Enable pyvenv global mode
...
(use-package company
  ...
  (setq company-idle-delay 0.1) ; Short delay for completion popup
  )
...
(custom-set-variables
 ...
 '(org-agenda-files
   '("/Users/joseph.savoie/org/inbox.org" "/Users/joseph.savoie/org/cycling/fit-utils.org" "/Users/joseph.savoie/org/cycling/cycling.org"))
 '(org-cycle-separator-lines 1)
 '(package-selected-packages
   '(jupyter eglot pyvenv speed-type vertico org-roam valign clojure-mode lsp-mode cider lsp-treemacs flycheck company))
